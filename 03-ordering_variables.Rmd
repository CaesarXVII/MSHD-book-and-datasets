...
In a sequential testing procedure in which the $p$-values $p_k,k=1,\ldots p$ (and associated null hypotheses) have been ordered in an ascending order, $\mathbb{H}_{0j}, j=k,\ldots,p$ are rejected if $p_k>1-\alpha_k$ with $$\alpha_k=\alpha(k+1)/p.$$ With this rejection rule, the FDR ($\leq\alpha$) is controlled  [@BeHo:95]. Hence, in a stepwise procedure for variable selection, the search stops at step $k-1$ if $p_k> 1-\alpha_k$, $p_k$ being associated to the variable to enter at step $k$.
