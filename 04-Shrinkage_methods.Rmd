...
The adaptive lasso is a *two-stage approach*, with at the first stage a chosen initial estimator which produces a single set of weights $w_j^{\gamma}$ which are held constant (for fixed $\gamma$) across all values of $\lambda$. An alternative approach, known as a *pathwise approach* is to let the weights change with $\lambda$: $w_j^{\gamma}:=w_j^{\gamma}\left(\hat{\beta}_j\right)$. A notable example is the *relaxed* lasso of @Mein:07 which, for $\lambda\geq 0$ and $0<\phi\leq 1$ is given by
\begin{equation}
