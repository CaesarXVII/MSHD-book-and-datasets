...
> Additional material:\
> Actually, $\hat{y}_{0i}$ and $\hat{y}_{(i)}$ have to be understood as *functions* applied respectively on $y_{0i}$ or $y_{i}$. The functions are the *prediction rules* built during the learning phase, i.e. respectively on $y_i,i=1,\ldots,n$ or $y_i,i=1,\ldots,i-1,i+1,n$.
...
> Exercise: (solutions (\@ref(cv)) provided by **Alexander Maslev**, **Hanxiong Wang** and **Minyoung Lee**).\ 
...
> Note: again, $\hat{Y}_i$ is understood as a function of the sample.
...
b. Compute $\hat{y}_i^{(j)}, i=1,\ldots,n$ (i.e. the prediction rule), possibly conditionally on the matrix of predictors $\mathbf{X}$\
4. Compute $\forall i$ 
\begin{equation}  
...
> - Compare the analysis (decision) with the one obtained by means of cross-validation (in the previous exercise).
...
> Exercise (solutions presented in \@ref(roc))\
