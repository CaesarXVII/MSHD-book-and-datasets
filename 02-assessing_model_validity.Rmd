...
<!-- Basically here we have two choices: either we demonstrate equality with $\hat{C}_p$ or we we look not at the estimator of the error $\widehat{\text{Err}}$ but at the true one (i.e. $\text{Err}$) as Efron did in both 2004 paper and 1986. This because it changes the role of the \sigma^2: in practice we work with an estimator given that \sigma^2 is always unknown. If needed I can change the demonstration with $\text{Err}$ as the only difference would be a constant n which does not count in model selection 

> Starting from equation \@ref(eq:err), we need to retrieve equation \@ref(eq:mallow) for the specific case of the linear regression. 

> As we can notice, the *in-sample* quantity (i.e. $\frac{1}{n}\sum_{i=1}^n\left(\hat{y}_i-y_i\right)^2$) it is common to both equations so that we can focus only on the *out-of-sample* quantity(i.e. $\frac{2}{n}\sum_{i=1}^n\widehat{\text{cov}}\left(\hat{Y}_i;Y_i\right)$). We make use of the trace operator (see [trace](https://en.wikipedia.org/wiki/Trace_(linear_algebra))) to work on the multivariate dimension of the problem:
\begin{equation}
\frac{2}{n}\sum_{i=1}^n\widehat{\text{cov}}\left(\hat{Y}_i;Y_i\right) = \frac{2}{n} \; tr( \widehat{\boldsymbol{cov}}(\boldsymbol{\hat{y}},\boldsymbol{y}))
\end{equation}

> Now we use the fact that, in the linear regression case $\boldsymbol{\hat{y}} = \boldsymbol{H}\boldsymbol{y}$ where the projection matrix $\boldsymbol{H} = \boldsymbol{X}\;(\boldsymbol{X}^{T}\boldsymbol{X})^{-1}\;\boldsymbol{X}^{T}$. We can than write that:
\begin{equation}
\frac{2}{n} \; tr( \widehat{\boldsymbol{cov}}(\boldsymbol{\hat{y}},\boldsymbol{y})) = \frac{2}{n} \; tr( \widehat{\boldsymbol{cov}}(\boldsymbol{Hy},\boldsymbol{y}))
\end{equation}

>At this point, knowing that the covariance is a linear operator and that $\boldsymbol{cov}(\boldsymbol{y},\boldsymbol{y}) = \boldsymbol{Var}(\boldsymbol{y})$ we can conclude that:
\begin{align}
\frac{2}{n} \; tr( \widehat{\boldsymbol{cov}}(\boldsymbol{Hy},\boldsymbol{y}))

& = \frac{2}{n} \; tr(\boldsymbol{H} \hat{\sigma}^2 I) \\

& = \frac{2}{n} \hat{\sigma}^2 tr(\boldsymbol{H}) \\

& = \frac{2}{n} \hat{\sigma}^2 p  
\end{align}
> which implies equality with equation \@ref(eq:mallow) in the case of $\sigma^{2}$ unknown.
> q.e.d.
-->
...
<!--
> Exercise: \
- Compute a ROC curve from a dataset with chosen predictors
-->
